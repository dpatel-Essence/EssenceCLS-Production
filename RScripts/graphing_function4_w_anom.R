graphing_function4_w_anom = function(df1= best_fit_non_search_chrome,df2 = Final_CLS_2022_Study_List_Non_Search_model_file_Chrome,i)
{
  p <- df1[[i]]["power"]
  ...
  p <-
    ...
    geom_point(
      ...
      aes(
        ...
        size = anomaly_score,
        text = paste(pa, channel, tactic, region, average_depth, study_id)
      )
    ) +
    geom_text(data = df2[[i]],aes(x = cost_spent_on_exposed_group, y = absolute_lift, label = round(anomaly_score * 100,digits = 1)),size = 3) +
