graphing_function3 = function(df1= best_fit_non_search_chrome,df2 = Final_CLS_2022_Study_List_Non_Search_model_file_Chrome,i)
{
  p <- df1[[i]]["power"]
  ...
  y_pred <-
    (mapply("*", mapply(`^`, df2[[i]]["cost_spent_on_exposed_group"], p), b1) +
       mapply("*", data.frame(mapply(`^`, df2[[i]]["cost_spent_on_exposed_group"], 2*p)), b2)) %>%
    as.data.frame()
