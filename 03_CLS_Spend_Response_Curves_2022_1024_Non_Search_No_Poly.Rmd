...
### Check Correlation between Spend and Study Duration

```{r}

cor(Final_CLS_2022_Study_List_Non_Search_model_file$duration,Final_CLS_2022_Study_List_Non_Search_model_file$cost_spent_on_exposed_group)


cor(Final_CLS_2022_Study_List_Non_Search_model_file$duration,Final_CLS_2022_Study_List_Non_Search_model_file$absolute_lift)
...
#### Testing Timing for each model
```{r, warning=FALSE}
fits_non_search_chrome <- list()
fits_non_search_chrome_test <-list()

start_time <- Sys.time()
for (i in 1:length(Final_CLS_2022_Study_List_Non_Search_model_file_chrome)) {
  fits_non_search_chrome[i] <- list(lapply(powers, ridge_lasso_function,
    parameter_grid = parameters, alpha_grid = alpha_parameters, cost = "cost_spent_on_exposed_group",
    response = "absolute_lift", data = Final_CLS_2022_Study_List_Non_Search_model_file_chrome[[i]]
  ))
}
end_time <- Sys.time()
...
fits_non_search_chrome_test <- lapply(1, function (i)
{
  lapply(powers2, function(j){
    list((lapply(powers, ridge_lasso_function4,powers2 = j,
    parameter_grid = parameters, alpha_grid = alpha_parameters, cost = "cost_spent_on_exposed_group",
    response = "absolute_lift", data = Final_CLS_2022_Study_List_Non_Search_model_file_chrome[[i]]
    )
    )
...
names(Final_CLS_2022_Study_List_Non_Search_model_file_chrome)
  ...
  fits_non_search_chrome_test<-
  (lapply(powers, ridge_lasso_function4, powers2 = j,
    parameter_grid = parameters, alpha_grid = alpha_parameters, cost = "cost_spent_on_exposed_group",
    response = "absolute_lift", data = Final_CLS_2022_Study_List_Non_Search_model_file_chrome[[i]]
    )
    )  
...
Final_CLS_2022_Study_List_Non_Search_model_file_chrome[[2]]["cost_spent_on_exposed_group"] %>% 
    as.data.frame() %>% 
    mutate_at(1,~(.^power)) %>% 
    rename(cost_p = 1) %>%
    mutate(cost_p2 = (cost_p^(1/power))^powers2) %>% 
    data.matrix()
  
          
fits_non_search_chrome <- lapply(1:length(Final_CLS_2022_Study_List_Non_Search_model_file_chrome), function(i) {
  lapply(powers2, function(j) {
    ((lapply(powers, ridge_lasso_function4,
      powers2 = j,
      parameter_grid = parameters, alpha_grid = alpha_parameters, cost = "cost_spent_on_exposed_group",
      response = "absolute_lift", data = Final_CLS_2022_Study_List_Non_Search_model_file_chrome[[i]], poly_ind = 1
    )
    ))
  })
})
...
best_ind_non_search_chrome <- 
  lapply(1:length(Final_CLS_2022_Study_List_Non_Search_model_file_chrome), function(i,df = fits_non_search_chrome)
  {
    df[[i]] %>%
  bind_rows() %>%
  slice_min(TrainRMSE) %>%
  mutate(
    model = names(Final_CLS_2022_Study_List_Non_Search_model_file_chrome)[i] 
  )
  } 
  )

names(Final_CLS_2022_Study_List_Non_Search_model_file_chrome)[3] 

Final_CLS_2022_Study_List_Non_Search_model_file_chrome[[i]] %>%
  names()

names(Final_CLS_2022_Study_List_Non_Search_model_file_chrome)[1] 


fits_non_search_chrome[[1]][[1]][[1]]

fits_non_search_chrome[[1]][[2]]

map(map(fits_non_search_chrome[[1]],1),1)

map(map(fits_non_search_chrome[[1]][[1]],1),1)
```
